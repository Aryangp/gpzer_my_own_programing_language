...
class Parser:
    ...
    def factor(self):
        ...
        if tok.type in (TT_PLUS,TT_MINUS):
            ...
            return res.success(UnaryOpNodes(tok,factor))
    ...
    def func_def(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD,"pokeball"):
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start,self.current_tok.pos_end,"Expected 'pokeball'"))
    ...
    def comp_expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD,"not"):
            ...
            if res.error: return res
            return res.success(UnaryOpNodes(op_tok,node))
        node=res.register(self.bin_op(self.arith_expr,(TT_EE,TT_NE,TT_LT,TT_GT,TT_LTE,TT_GTE)))
        if res.error:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start,self.current_tok.pos_end,"Expected int or float, Indentifier, '+','-','(',not"))
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD,"pikachu"):
            ...
            expr=res.register(self.expr())
            if res.error: return res
            return res.success(VarAssignNodes(var_name,expr))
...
class Function(Value):
	...
	def execute(self, args):
		...
		new_context = Comtext(self.name, self.context, self.pos_start)
...
class Interpreter:    
    ...
    def visit_BinOpNodes(self,node,context):
        res=RTResult()
    ...
    def visit_UnaryOpNodes(self,node,context):
        res=RTResult()
