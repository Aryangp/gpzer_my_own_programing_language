...
class Parser:
  ...
  def func_def(self):
    ...
    if not self.current_tok.matches(TT_KEYWORD, 'pokeball'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'FUN'"
      ))
...
class BaseFunction(Value):
    ...
    def check_args(self,args_names,args):
        ...
        if len(args) > len(args_names):
              return res.failure(RTError(self.pos_start,self.pos_end,f"{len(args)-len(args_names)} too many args passed into '{self.name}'",self.context))
        if len(args) < len(args_names):
              return res.failure(RTError(self.pos_start,self.pos_end,f"{len(args_names)-len(args)} too few args passed into '{self.name}'",self.context))
        return res.success(None)

    def populate_args(self,args_names,args,exec_ctx):
    ...
    def check_populate(self,args_names,args,exec_ctx):
        res=RTResult()
...
class Function(BaseFunction):
  ...
  def execute(self, args):
    ...
    exc_ctx= self.generate_new_context()

    res.register(self.check_populate(self.arg_names,args,exc_ctx))
    if res.should_return(): return res

    value = res.register(interpreter.visit(self.body_node, exc_ctx))
...
class BuildInFunction(BaseFunction):  
    ...
    def execute(self, args):
      ...
      res.register(self.check_populate(method.arg_names,args,exec_ctx))
      if res.should_return():return res
    ...
    def execute_go(self,exec_ctx):
       print(str(exec_ctx.symbol_table.get("value")))
       return RTResult().success(Number.null)
    execute_go.arg_names=['value']

    def execute_go_ret(self,exec_ctx):
       return RTResult().success(String(str(exec_ctx.symbol_table.get("value"))))
    execute_go_ret.arg_names=['value']

    def execute_capture(self,exec_ctx):
       text=input()
       return RTResult().success(String(text))
    execute_capture.arg_names=[]
...
BuildInFunction.go=BuildInFunction("go")
BuildInFunction.go_ret=BuildInFunction("go_ret")
BuildInFunction.capture=BuildInFunction("capture")
...
class Interpreter:
  ...
  def visit_ForNode(self, node, context):
    ...
    while condition():
      ...
      if res.should_return() and res.loop_should_break == False and res.loop_should_continue==None: return res
...
global_symbol_table.set("go", BuildInFunction.go)
global_symbol_table.set("go_ret", BuildInFunction.go_ret)
global_symbol_table.set("capture", BuildInFunction.capture)
